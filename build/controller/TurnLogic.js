...
var Clock_1 = require("./Utility/Clock");
var Interface_1 = require("../view/Interface");
var Scorer_1 = require("../model/Scorer");
var OpponentLogic_1 = require("./OpponentLogic");
...
var TurnLogic = (function () {
    function TurnLogic() {
        ...
        this._interface = new Interface_1.default();
        this._clock = new Clock_1.default();
        this._table = new Table_1.default();
        this._opponent = [new Player_1.default(), new Player_1.default()];
        this._player = new User_1.default();
        this._opponent[1] = new Player_1.default();
    }
    Object.defineProperty(TurnLogic.prototype, "state", {
        get: function () {
            return this._state;
        },
        enumerable: true,
    ...
    TurnLogic.prototype._resetFlag = function () {
        for (var i = 0; i < this._flag.length; i++) {
            this._flag[i] = true;
        }
    };
    TurnLogic.prototype._setInterface = function () {
    ...
    TurnLogic.prototype._startGame = function () {
        if (this._flag[0]) {
            this._flag[0] = false;
            this._clock.reset();
            if (this._table.empty()) {
                ...
                for (var i = 0; i < this._opponent.length; i++) {
            ...
            this._opponent[0].scoreView.string("$" + this._opponent[0].score.toString());
            this._player.scoreView.string("$" + this._player.score.toString());
            this._opponent[1].scoreView.string("$" + this._opponent[1].score.toString());
        }
    ...
    TurnLogic.prototype._preRound = function () {
        if (this._flag[0]) {
            this._flag[0] = false;
            this._clock.reset();
        ...
        if (!this._opponent[1].gameOver) {
            if (this._flag[6] && this._clock.get.seconds() > 3.5) {
                this._flag[6] = false;
                this._opponent[1].card1.card = this._opponent[1].cardAt(0);
                this._opponent[1].card1.default();
            }
            if (this._flag[7] && this._clock.get.seconds() > 4) {
                this._flag[7] = false;
                this._opponent[1].card2.card = this._opponent[1].cardAt(1);
    ...
    TurnLogic.prototype._tableFirstRound = function () {
        if (this._flag[0]) {
            this._flag[0] = false;
            this._clock.reset();
    ...
    TurnLogic.prototype._tableSecondRound = function () {
        if (this._flag[0]) {
            this._flag[0] = false;
            this._clock.reset();
    ...
    TurnLogic.prototype._tableThirdRound = function () {
        if (this._flag[0]) {
            this._flag[0] = false;
            this._clock.reset();
            this._table.dealToTable(1);
    ...
    TurnLogic.prototype._scoreRound = function () {
        if (this._flag[0]) {
            this._flag[0] = false;
            this._clock.reset();
        }
        ...
        if (this._flag[2] && this._clock.get.seconds() > 1) {
            this._flag[2] = false;
            var winner = this._player;
            var tmp = 0;
            for (var i = 0; i < this._opponent.length; i++)
                if (this._opponent[i].gameOver || this._opponent[i].fold)
                    tmp++;
            if (this._player.fold)
                tmp++;
            if (tmp == this._opponent.length) {
                ...
                Scorer_1.default.assignHandScores(this._table);
                winner = Scorer_1.default.compareHandScores(this._table);
            }
            ...
            for (var i = 0; i < this._opponent.length; i++)
                if (!this._opponent[i].gameOver) {
                    this._opponent[i].score === 0 ? this._opponent[i].scoreView.string("BUST") :
                        this._opponent[i].scoreView.string("$" + this._opponent[i].score.toString());
            ...
            if (winner === this._player) {
                ...
                if (winner === this._opponent[0])
                    ...
                    this._opponent[1].scoreMarker.string('&#x2192');
            }
        }
    ...
    TurnLogic.prototype._finalRound = function () {
        for (var i = 0; i < this._opponent.length; i++)
            if (!this._opponent[i].gameOver) {
        ...
        if ((this._opponent[0].gameOver && this._opponent[1].gameOver) || this._player.score <= 0) {
            this._state++;
        }
        else {
            this._state = 0;
    ...
    TurnLogic.prototype._gameOver = function () {
        if (this._flag[0]) {
            this._flag[0] = false;
    ...
    TurnLogic.prototype._playerRound = function () {
        if (this._flag[0]) {
            this._flag[0] = false;
            this._clock.reset();
        ...
        if (this._flag[1] && this._clock.get.seconds() > 0.5) {
            ...
            if (this._state == 2) {
                ...
                this._player.turnView.color("green");
            }
            else {
    ...
    TurnLogic.prototype._opponentRound = function (opponent) {
        if (this._flag[0]) {
            this._flag[0] = false;
            this._clock.reset();
        ...
        if (this._flag[3] && this._clock.get.seconds() > 2) {
            this._flag[3] = false;
            var tmp = OpponentLogic_1.default.play(opponent, this._table);
            if (tmp > 0) {
    ...
    TurnLogic.prototype._switchBettingRound = function () {
        if (this._table.callAmount !== 0 && !this._opponent[0].gameOver && !this._table.skipTurn(this._opponent[0])) {
            this._betState = 0;
        }
        else if (this._table.callAmount !== 0 && !this._table.skipTurn(this._player)) {
            this._betState = 1;
        }
        else if (this._table.callAmount !== 0 && !this._opponent[1].gameOver && !this._table.skipTurn(this._opponent[1])) {
            this._betState = 2;
        }
        else {
            ...
            this._betState = 0;
            var tmp = 0;
            if (this._opponent[0].gameOver || this._opponent[0].fold)
                tmp++;
            if (this._opponent[1].gameOver || this._opponent[1].fold)
                tmp++;
            if (this._player.fold)
                tmp++;
    ...
    TurnLogic.prototype._bettingRound = function () {
    ...
    TurnLogic.prototype.playerRaise = function () {
        if (this._clickable) {
            ...
            if (this._player.score === 0) {
                ...
                this._player.currentBetAmount = 10;
                this._interface.raiseAmount.string("$" + this._player.currentBetAmount.toString());
            }
    ...
    TurnLogic.prototype.playerCall = function () {
        if (this._clickable) {
    ...
    TurnLogic.prototype.playerFold = function () {
        if (this._clickable) {
    ...
    TurnLogic.prototype.incrementBetAmount = function () {
        var tmp = this._player.currentBetAmount + 10;
        if (this._clickable && tmp + this._table.callAmount <= this._player.score) {
            ...
            this._interface.raise.string("$" + (this._table.callAmount + tmp).toString() + "\nRAISE");
            this._player.currentBetAmount = tmp;
        }
    };
    TurnLogic.prototype.decrementBetAmount = function () {
        var tmp = this._player.currentBetAmount - 10;
    ...
    TurnLogic.prototype.gameDriver = function () {
        switch (this._state) {
            case 0:
                this._startGame();
                break;
